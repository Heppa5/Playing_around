...
Mat calc_pose_error_old(Mat current, Mat desired) // transformation matrices
{
...
{
    Mat error=Mat::zeros(6,1,CV_32F);
    error(Rect(0,0,1,3))=current(Rect(3,0,1,3))-desired(Rect(3,0,1,3));
    Mat commonRcurrent=current(Rect(0,0,3,3));
    Mat commonRdesired=desired(Rect(0,0,3,3)),desiredRcommon;
    transpose(commonRdesired,desiredRcommon);
    Mat desiredRcurrent=desiredRcommon*commonRcurrent,desiredRcurrent_EAA;
//    cout << currentRdesired << endl;
    Rodrigues(desiredRcurrent,desiredRcurrent_EAA);
//    cout << currentRdesired_EAA << endl;
//    error(Rect(0,3,1,3))=currentRdesired_EAA;
    error.at<float>(3,0)=desiredRcurrent_EAA.at<float>(0,0);
    error.at<float>(4,0)=desiredRcurrent_EAA.at<float>(1,0);
    error.at<float>(5,0)=desiredRcurrent_EAA.at<float>(2,0);
//    cout << error(Rect(0,3,1,3)) << endl;
    return error;
}
...
//        Mat error=calc_pose_error_old(cTm1,cTm2);
        Mat error=calc_pose_error(cTm1,cTm2);
        cout << "Current error \n" << error << endl;
//        cout << "Old calc of error: \n" << calc_pose_error_old(cTm1,cTm2) << endl;

        cout << "cTm1 before correction is: \n" << cTm1 << endl;



        Mat error_correction=Mat::zeros(6,1,CV_32F);
//        error_correction(Rect(0,0,1,3))=correction*(error(Rect(0,0,1,3))+make_skew_mat(cTm1(Rect(3,0,1,3)))*error(Rect(0,3,1,3)));  // works with old_error_calc
        error_correction(Rect(0,0,1,3))=correction*((cTm2(Rect(3,0,1,3))-cTm1(Rect(3,0,1,3)))+make_skew_mat(cTm1(Rect(3,0,1,3)))*error(Rect(0,3,1,3)));
        error_correction(Rect(0,3,1,3))=-correction*error(Rect(0,3,1,3));
